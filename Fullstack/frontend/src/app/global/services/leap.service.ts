...
export class LeapService {
  ...
  private pinchEvent: EventEmitter<Gestures>;

  /*****************************/
  public gestureRecognized: string;
  public eventRecognized: string;
  private allowPinch: boolean = true;
  /*****************************/
  ...
  constructor() {
    ...
    Leap.loop(controllerOptions, (frame) => {
      ...
      var pinch = this.findPinch(frame);

      if(pinch === Gestures.PINCH){
        this.gestureEvent.emit(pinch);
        this.eventRecognized = 'pinch';
        pinch = null;
      }

      if (gesture != null){
        this.gestureEvent.emit(gesture);
        this.eventRecognized = 'gesture';
      }
  ...
  public gestureRecognizer(): Observable<Gestures> {
    if(this.eventRecognized === "gesture"){
      this.eventRecognized = null;
      return this.gestureEvent;
    }
    else if(this.eventRecognized === "pinch"){
      this.eventRecognized = null;
  ...
  private findPinch(frame) {
    if (frame.data.hands.length > 0 && this.allowPinch && frame.data.hands[0].pinchStrength === 1) { //depends on the accuracy you want. Max and more accurate is 1!
  ...
  private initializeEmmiters() {
    ...
    this.pinchEvent = new EventEmitter<Gestures>();
  }
